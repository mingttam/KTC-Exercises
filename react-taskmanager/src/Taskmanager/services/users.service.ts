...
export const addRoleToUser = async (userId: number, roleId: number) => {
  const result = await apiClient.put(`/security/users/${userId}/add-role-to-user`, { role_id: roleId });
  return result;
};

export const removeRoleFromUser = async (userId: number, roleId: number) => {
  const result = await apiClient.put(`/security/users/${userId}/remove-role-from-user`, { role_id: roleId });
  return result;
};